...
function allowed(tail: readonly Clip[], clip: Clip): boolean {
  ...
  if (!last) return true
  if (clip.work === last.work) return false
  if (trailingKindRun(tail) >= RULES.maxSameKindRun && clip.isAI === last.isAI) return false
  return true
}
 ...
 * 極端に偏ったプール（同じ作品しかない等）では、満たせない制約を落とした並びを返す。
 */
export function planCycle(pool: readonly Clip[], rng: Rng, prevTail: readonly Clip[]): Clip[] {
  ...
  // 制約を満たす並びが存在しないプール。守れるものだけ守って並べる
  const remaining = [...pool]
  const tail = [...prevTail]
  const out: Clip[] = []
  while (remaining.length > 0) {
    ...
    const loose =
      strict.length > 0
        ? strict
        : remaining.filter((c) => c.id !== tail[tail.length - 1]?.id)

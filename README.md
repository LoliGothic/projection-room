...
`drawbox` で縦スジを数本描き、数コマおきに現れるようにしています。位置・濃さ・出方は
動画IDから決まるので、同じ動画なら毎回同じ傷が入ります。**本物にも同じ範囲で入れます。**

濃さは 0.15〜0.35 程度。これ以上濃くすると傷が主役になってしまいます。
なお傷は数コマおきにしか出ないため測定値が安定せず、ここだけは閉ループにせず
固定の強さで入れています。

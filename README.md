...
- **右スワイプ / →** … 残す（本物だと思ったとき）
- **左スワイプ / ←** … 報告する（AI生成だと思ったとき）
- **右側の ↻** … リプレイ。回数に制限も罰もありません
...
実素材がまだないため、ffmpeg のテストパターンから **本物役10本・AI役10本** を生成しています。
動作確認しやすいよう本物役は落ち着いた色、AI役は彩度が高く滑らか、と見た目を分けてあります。
...
mp4 は `public/clips/` に出力され、**git 管理外**です。
CI は実素材がリポジトリに無ければダミーを生成してから配信します。
ID はシードから決まるので、コミット済みの `clips.json` と必ず一致します。
...
3. 提供者・出典を `public/clips.json` に書き足します（クレジット画面と振り返り画面に出ます）。

4. 実素材に差し替えたら、`.gitignore` の `/public/clips/*.mp4` を外して mp4 をコミットしてください。

...
| 役 | 中身 |
| --- | --- |
| **本物** 38本（13作品） | `clip_picker.py` で選んで `saved/` に切り出したパブリックドメイン映画 |
| **AI** 10本 | Seedance 2.0 Mini で生成した1920年代スラップスティック風の動画 |
...
まず 18fps / 640x480 / モノクロ / 無音 に揃えたうえで、**1本ずつ次の 2 つを行います**。

出力は 640x480 です。スマホでは 4:3 が幅いっぱい（430px 前後、実画素はその2〜3倍）に
広がるため、480x360 だと引き伸ばしで眠くなります。
...
- **左上の「上映をやめる」** … タイトルに戻ります。誤タップ防止のため2回押す必要があります
